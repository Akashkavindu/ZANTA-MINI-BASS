...
const sms = (zanta, m) => {
	...
	if (m.message) {
		...
		if (m.msg) {
			...
			// --- m.body (UPDATED FOR LIST & INTERACTIVE BUTTONS) ---
			...
			if (m.quoted) {
                ...
                // --- m.quoted.body (UPDATED FOR LIST & INTERACTIVE BUTTONS) ---
				m.quoted.body = (m.quoted.type === 'conversation') ? m.quoted.msg : 
                                (m.quoted.type === 'extendedTextMessage') ? m.quoted.msg.text : 
                                (m.quoted.type == 'imageMessage') && m.quoted.msg.caption ? m.quoted.msg.caption : 
                                (m.quoted.type == 'videoMessage') && m.quoted.msg.caption ? m.quoted.msg.caption : 
                                (m.quoted.type == 'templateButtonReplyMessage') && m.quoted.msg.selectedId ? m.quoted.msg.selectedId : 
                                (m.quoted.type == 'buttonsResponseMessage') && m.quoted.msg.selectedButtonId ? m.quoted.msg.selectedButtonId : 
                                (m.quoted.type == 'listResponseMessage') && m.quoted.msg.singleSelectReply.selectedRowId ? m.quoted.msg.singleSelectReply.selectedRowId : 
                                (m.quoted.type == 'interactiveResponseMessage') ? JSON.parse(m.quoted.msg.nativeFlowResponseMessage.paramsJson).id : ''
